...
The hash that we get is an NTLMv2 hash, which we can crack e.g. with hashcat with the ```-m 5600``` hashmode. With the recovered clear text credentials we can then proceed to move laterally.  
...
A word of warning at this stage: Be aware that the client will afterwards route all his browser HTTP traffic to you attacker machine which might break stuff on the network.  
...
This again will give us the NTLMv2 hash.  
